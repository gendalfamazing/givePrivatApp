...
struct Prikaz1030Alg36ViewFavorites: View {
    ...
    var isInFavorites: Bool {
        return favoritesManager.favorites.contains { $0.viewIdentifier == "Алгоритм 36. «Почечная колика»" }
    }
    
    func addToFavorites() {
        let newItem = FavoriteItem(name: "Приказ №1030", viewIdentifier: "Алгоритм 36. «Почечная колика»", isExpanded: false, isNavigationLink: true)
    ...
    func removeFromFavorites() {
        if let item = favoritesManager.favorites.first(where: { $0.viewIdentifier == "Алгоритм 36. «Почечная колика»" }) {
